...
class AC17CPABE(ABEnc):
    ...
    def encrypt(self, pk, msg, policy_str):
        ...
        skey = self.group.random(GT)
        cipher = AuthenticatedCryptoAbstraction(extract_key(skey))
        c_msg = cipher.encrypt(msg)
        
        # compute the e(g, h)^(k^T As)
        Cp = 1
        for i in range(self.assump_size):
            Cp = Cp * (pk['e_gh_kA'][i] ** s[i])
        Cp = Cp * skey

        return {'policy': policy, 'C_0': C_0, 'C': C, 'Cp': Cp, 'c_msg':c_msg}
    ...
    def decrypt(self, pk, ctxt, key):
        ...
        skey = ctxt['Cp'] * prod2_GT / prod1_GT
        cipher = AuthenticatedCryptoAbstraction(extract_key(skey))
        ptxt = cipher.decrypt(ctxt['c_msg'])
